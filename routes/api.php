...
Route::middleware('auth:api')->prefix('clientes')->group(function () {
    ...
    Route::get('/{id}', [ClienteController::class, 'show']); // Ver cliente por ID
});

// Ruta de dashboard solo para administradores
...
Route::middleware('auth:api')->prefix('administracion')->group(function () {
    ...
    Route::prefix('productos')->group(function () {
        Route::get('/', [CtlProductosController::class, 'index'])->middleware('rol:Admin'); // Lista de productos
        Route::get('/filtrados', [CtlProductosController::class, 'getProductosFiltrados'])->middleware('rol:Admin'); // Productos filtrados
        Route::get('/{id}', [CtlProductosController::class, 'show'])->middleware('rol:Admin'); // Obtener producto por ID
...
Route::prefix('catalogo')->group(function () {
    ...
    Route::get('/productos', [CtlProductosController::class, 'index']); // Aquí tal vez quieras agregar filtros si es necesario
    Route::get('/productos/filtrados', [CtlProductosController::class, 'getProductosFiltrados']); // Productos filtrados para catálogo
});
